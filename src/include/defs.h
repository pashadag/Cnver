...
void open_file_binary(ifstream & file, string filename) {
...
void read_file_into_vector(string filename, vector<string> & data) {
	string sbuf;
	ifstream inf;
	open_file(inf, filename);
	while (getline(inf, sbuf)) data.push_back(sbuf);
	return;
}
