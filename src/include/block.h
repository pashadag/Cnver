...
//bps are the set of breakpoints and are updated as new ones are created
//the new ones are also put into newbps as a pass-back parameter.
//only the main_idx interval of each block is checked for overlaps with bps.
...
void split_blocks(vector<Block> & blocks, set<int> &bps, set<int> & newbps) {
	// Assume that there are no gaps (this case would be difficult to handle properly 
	// and would require more information in the alignment)
	int maxBlockSize = 0;
